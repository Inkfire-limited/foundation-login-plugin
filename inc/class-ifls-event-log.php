...
class IFLS_Event_Log {
    ...
    /**
     * Count events matching the same filters as query().
     *
     * @param array $args event, outcome, search.
     * @return int
     */
    public static function count(array $args = []) {
        ...
        try {
            ...
            if (!empty($args['event']) && is_scalar($args['event'])) {
                $where[]  = 'event = %s';
                $params[] = (string) $args['event'];
            }

            if (!empty($args['outcome']) && is_scalar($args['outcome'])) {
                $where[]  = 'outcome = %s';
                $params[] = (string) $args['outcome'];
            }
     ...
     *
     * @return bool True when the table was truncated.
     */
    public static function clear() {
        global $wpdb;

        try {
            return false !== $wpdb->query('TRUNCATE TABLE ' . self::table());
        } catch (\Throwable $e) {
            return false;
        }
    }
