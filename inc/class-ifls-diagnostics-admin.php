...
class IFLS_Diagnostics_Admin {
    ...
    public static function handle_clear_log() {
        ...
        $confirmed = isset($_POST['ifls_confirm_clear_log'])
            && is_scalar($_POST['ifls_confirm_clear_log'])
            && '1' === (string) wp_unslash($_POST['ifls_confirm_clear_log']);

        if (!$confirmed) {
            wp_safe_redirect(self::page_url(['ifls_clear_confirmation_required' => 1]));
            exit;
        }

        $cleared = IFLS_Event_Log::clear();

        wp_safe_redirect(self::page_url(['ifls_cleared' => $cleared ? 1 : 0]));
        exit;
    }
    ...
    public static function render() {
        ...
        $filter_event = isset($_GET['ifls_event']) && is_scalar($_GET['ifls_event'])
            ? sanitize_key(wp_unslash($_GET['ifls_event']))
            : '';
        $search = isset($_GET['ifls_search']) && is_scalar($_GET['ifls_search'])
            ? sanitize_text_field(wp_unslash($_GET['ifls_search']))
            : '';

        if ($filter_event && !in_array($filter_event, IFLS_Event_Log::EVENTS, true)) {
            $filter_event = '';
        }

        $per_page    = 50;
        $query_args  = ['event' => $filter_event, 'search' => $search];
        $total       = IFLS_Event_Log::count($query_args);
        $total_pages = max(1, (int) ceil($total / $per_page));
        $paged       = isset($_GET['ifls_paged']) && is_scalar($_GET['ifls_paged'])
            ? max(1, absint($_GET['ifls_paged']))
            : 1;
        $paged       = min($paged, $total_pages);
        $rows        = IFLS_Event_Log::query(array_merge($query_args, [
            'limit'  => $per_page,
            'offset' => ($paged - 1) * $per_page,
        ]));
        ?>
        <div class="wrap ifls-diagnostics-wrap">
            ...
            <?php if (isset($_GET['ifls_cleared']) && is_scalar($_GET['ifls_cleared']) && '1' === (string) $_GET['ifls_cleared']) : ?>
                <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Event log cleared.', 'inkfire-login-styler'); ?></p></div>
            <?php endif; ?>

            <?php if (isset($_GET['ifls_cleared']) && is_scalar($_GET['ifls_cleared']) && '0' === (string) $_GET['ifls_cleared']) : ?>
                <div class="notice notice-error"><p><?php esc_html_e('The event log could not be cleared. Please try again.', 'inkfire-login-styler'); ?></p></div>
            <?php endif; ?>

            <?php if (isset($_GET['ifls_clear_confirmation_required'])) : ?>
                <div class="notice notice-warning"><p><?php esc_html_e('Confirm that you understand the event log will be permanently deleted before clearing it.', 'inkfire-login-styler'); ?></p></div>
            <?php endif; ?>
            ...
            <?php else : ?>
                <div class="ifls-diagnostics-table-wrap">
                <table class="widefat striped">
                    <thead><tr>
                        <th scope="col"><?php esc_html_e('Incident', 'inkfire-login-styler'); ?></th>
                        <th scope="col"><?php esc_html_e('Reason', 'inkfire-login-styler'); ?></th>
                        <th scope="col"><?php esc_html_e('Count', 'inkfire-login-styler'); ?></th>
                        <th scope="col"><?php esc_html_e('Last seen', 'inkfire-login-styler'); ?></th>
                        <th scope="col"><?php esc_html_e('Status', 'inkfire-login-styler'); ?></th>
                    </tr></thead>
                    <tbody>
                    <?php
            ...
            <div class="ifls-diagnostics-table-wrap">
            <table class="widefat striped ifls-mail-diagnostics-table">
                <tbody>
            ...
            <form method="get" class="ifls-diagnostics-filter">
                <input type="hidden" name="page" value="<?php echo esc_attr(self::PAGE_SLUG); ?>">
                <label class="screen-reader-text" for="ifls-event-filter"><?php esc_html_e('Event type', 'inkfire-login-styler'); ?></label>
                <select id="ifls-event-filter" name="ifls_event">
                ...
                <label class="screen-reader-text" for="ifls-event-search"><?php esc_html_e('Search event log', 'inkfire-login-styler'); ?></label>
                <input id="ifls-event-search" type="search" name="ifls_search" value="<?php echo esc_attr($search); ?>" placeholder="<?php esc_attr_e('user or IP', 'inkfire-login-styler'); ?>">
                <?php submit_button(__('Filter', 'inkfire-login-styler'), 'secondary', '', false); ?>
            </form>

            <div class="ifls-diagnostics-table-wrap">
            <table class="widefat striped ifls-event-log-table">
                <thead><tr>
                    <th scope="col"><?php esc_html_e('When', 'inkfire-login-styler'); ?></th>
                    <th scope="col"><?php esc_html_e('Event', 'inkfire-login-styler'); ?></th>
                    <th scope="col"><?php esc_html_e('User', 'inkfire-login-styler'); ?></th>
                    <th scope="col"><?php esc_html_e('IP', 'inkfire-login-styler'); ?></th>
                    <th scope="col"><?php esc_html_e('Outcome', 'inkfire-login-styler'); ?></th>
                    <th scope="col"><?php esc_html_e('Agent', 'inkfire-login-styler'); ?></th>
                </tr></thead>
                <tbody>
                <?php if (empty($rows)) : ?>
            ...
            <?php if ($total > 0) : ?>
                <p class="description">
                    <?php
                    printf(
                        /* translators: 1: first event number, 2: last event number, 3: total matching events. */
                        esc_html__('Showing %1$d–%2$d of %3$d matching events.', 'inkfire-login-styler'),
                        (($paged - 1) * $per_page) + 1,
            ...
            <?php if ($total_pages > 1) : ?>
                ...
                $pagination_base = str_replace(
                    '999999999',
                    '%#%',
                    esc_url_raw(self::page_url([
                        'ifls_event'  => $filter_event,
                        'ifls_search' => $search,
                        'ifls_paged'  => 999999999,
                    ]))
                );
                $pagination = paginate_links([
                    ...
                    'total'     => $total_pages,
                    'type'      => 'list',
                    'prev_text' => __('Previous page', 'inkfire-login-styler'),
                    'next_text' => __('Next page', 'inkfire-login-styler'),
                ]);
                ?>
                <?php if ($pagination) : ?>
                    <nav class="tablenav-pages ifls-pagination" aria-label="<?php esc_attr_e('Event log pagination', 'inkfire-login-styler'); ?>">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top:12px"
                ...
                <label class="ifls-diagnostics-confirmation">
                    <input type="checkbox" name="ifls_confirm_clear_log" value="1" required>
                    <span><?php esc_html_e('I understand this permanently deletes all logged events.', 'inkfire-login-styler'); ?></span>
                </label>
